...
The Gameboy Advance has two memory sections: a small and fast internal work ram (IWRAM), and a much larger block of slightly slower external work ram (EWRAM). Most of the 32kB IWRAM is currently reserved for the engine, leaving 256kB for Lua code and data. The engine also owns all of the GBA's RAM and memory-mapped registers, so you should not write to any of these addresses.
